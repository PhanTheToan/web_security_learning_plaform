...
export function ActivityChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const res = await fetch(`${apiBaseUrl}/lab/solved-statistics`, { credentials: "include" })
  ...
  return (
    <div className="bg-gradient-to-br from-[#ffffff]/8 via-[#9747ff]/5 to-[#5a5bed]/8 backdrop-blur-sm p-6 rounded-xl border border-[#ffffff]/10 hover:border-[#9747ff]/60 transition-all duration-300 h-full shadow-[0_0_15px_rgba(151,71,255,0.15)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Lab Activity</h3>
          <p className="text-sm text-white/70">Select a date range to view activity.</p>
        </div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        {isLoading ? (
          <div className="w-full h-full flex items-center justify-center text-white/70">Loading Chart...</div>
        ) : chartData.length === 0 ? (
          <div className="w-full h-full flex items-center justify-center text-white/70">No data available for this period.</div>
        ) : (
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Line
              ...
              activeDot={{ r: 7, fill: "#22c55e", filter: "drop-shadow(0 0 8px #22c55e)" }}
            />
            <Line
              type="monotone"
              name="Expired"
              ...
              strokeWidth={3}
              dot={{ r: 4, fill: "#f97316", stroke: "hsl(var(--background))", strokeWidth: 2 }}
              activeDot={{ r: 7, fill: "#f97316", filter: "drop-shadow(0 0 8px #f97316)" }}
            />
          </LineChart>
        )}
      </ResponsiveContainer>
